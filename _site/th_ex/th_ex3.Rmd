...
travel_to_work <- travel %>%
  ...
  select(participantId, travelEndTime, year, year_month, month, day, date, travelEndLocationId, purpose, location, buildingId) %>%
...
travel_to_work_daily_count <- travel_to_work %>%
  group_by(employerId, year_month, day, date) %>%
  summarise(count = n()) %>%
  ungroup()
```
...
travel_to_work_by_day <- travel_to_work_daily_count %>%
  filter(year_month %in% c('2022-03', '2022-04')) %>%
  group_by(date) %>%
...
```{r}
p3 <- ggplot(travel_to_work_by_day, aes(date, daily_employees)) +
  geom_line()
