...
We will add a date column in order to track daily activities. We have also created a new column called *status* to incorporate "Sleeping" into *currentMode* which expands the original 5 distinct values to 6.

```{r}
logs_two_p$date <- date(logs_two_p$`timestamp`)

logs_two_p$status <- ifelse(logs_two_p$sleepStatus == 'Sleeping', 'Sleeping', logs_two_p$currentMode)
unique(logs_two_p$status)
```
...
logs_daily_ss <- logs_two_p %>%
  group_by(date, participantId, status) %>%
  summarise(count = n()) %>%
  mutate(ss = count * 60) %>%
  ungroup() %>%
  select(date, participantId, status, ss) %>%
  pivot_wider(names_from=status, values_from=ss,
              values_fill=0)
```
...
# ```{r}
# visielse(logs_daily_ss, group = logs_daily_ss$participantId, informer = NULL, method = "cut")
# ```
